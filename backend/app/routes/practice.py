...
#     """
#     Programmatic verifier for all 75 problems across all levels.
#     Computes the correct answer from input_data independently in Python,
...
#             is_in_output = 'leap' in output_lower and 'not' not in output_lower
#             return is_in_output == is_leap
...
#             exp = 'upper' if ch.isupper() else 'lower' if ch.islower() else 'digit' if ch.isdigit() else 'special'
...
#             return str(len(input_data.strip().lstrip('-'))) in nums
...
#             if op == '/' and b == 0:
#                 return any(w in output_lower for w in ['zero', 'cannot', 'divide', 'undefined'])
#             res = {'+':(a+b), '-':(a-b), '*':(a*b), '/':(a/b if b!=0 else None)}.get(op)
#             return has_num(res, 0.01) if res is not None else False
...
#             return ('not' not in output_lower and 'prime' in output_lower) == is_p
...
#             s = input_data.strip()
#             neg = s.startswith('-')
#             rev = s.lstrip('-')[::-1].lstrip('0') or '0'
#             return ('-'+rev if neg else rev) in output

#         if title == 'palindrome number':
#             s = input_data.strip()
#             is_p = s == s[::-1]
#             return ('not' not in output_lower and 'palindrome' in output_lower) == is_p

#         if title == 'armstrong number':
#             s = input_data.strip()
#             d = len(s)
#             is_a = sum(int(c)**d for c in s if c.isdigit()) == int(s)
#             return ('not' not in output_lower and 'armstrong' in output_lower) == is_a
...
#             return ('not' not in output_lower and 'anagram' in output_lower) == is_a
...
#                 comp += s[i] + str(count)
#                 i += count
#             return (s if len(comp) >= len(s) else comp) in output
...
#             return has_num(sum(input_nums()), 0.01)
...
#             grade_ok = ('fail' in output_lower or 'f' in output_lower) if g == 'f' else g in output_lower
#             return grade_ok and str(total) in nums
...
#             if age < 18:
#                 return any(w in output_lower for w in ['18', 'invalid', 'exception', 'must', 'above', 'error'])
#             return any(w in output_lower for w in ['granted', 'valid', 'welcome', 'success'])

#         if title == 'number guessing game':
#             return any(w in output_lower for w in ['high', 'low', 'correct', 'guess', 'attempt'])
...
#                 return any(w in output_lower for w in ['denied', 'incorrect', 'invalid', 'wrong'])
#             if action == '1':
...
#             return any(w in output_lower for w in ['score', 'correct', 'wrong', 'question'])
...
#                 return any(w in output_lower for w in ['not found', 'invalid'])
...
#             if action == '2':
#                 return any(w in output_lower for w in ['restocked', 'updated']) or len(nums) > 0
#             if action == '3':
...
#             action = il[0] if il else ''
#             if action == '1':
#                 return any(w in output_lower for w in ['added', 'order'])
#             if action == '2':
#                 return any(w in output_lower for w in ['removed', 'not found'])
#             if action == '4':
...
#             if action == '2':
#                 if 'wrong' in input_data.lower() or 'invalid' in input_data.lower():
#                     return any(w in output_lower for w in ['invalid', 'incorrect', 'locked', 'attempt'])
#                 return any(w in output_lower for w in ['welcome', 'success', 'invalid', 'not found']) or username.lower() in output_lower
#             return len(output) > 0
...
#                     sort_choice = il[1] if len(il) > 1 else '1'
#                     return salary_nums == sorted(salary_nums) if sort_choice == '1' else salary_nums == sorted(salary_nums, reverse=True)
#                 return len(lines) > 0
#             if action == '4':
...
#             if action == '3':
#                 return any(w in output_lower for w in ['invoice', 'total', 'checked out']) or len(nums) > 0
#             if action == '4':
...
#                 return val in nums or any(w in output_lower for w in ['overflow', 'full', 'stack'])
#             if action == '2':
...
#             action = il[0] if il else ''
#             if action == '1':
#                 return any(w in output_lower for w in ['queued', 'added', 'full', 'enqueue'])
#             if action == '2':
#                 return any(w in output_lower for w in ['processing', 'dequeued', 'empty']) or len(output) > 0
#             if action == '3':
#                 return len(output) > 0 or 'empty' in output_lower
#             if action == '4':
#                 return any(w in output_lower for w in ['print', 'job']) or len(lines) > 0
#             return len(output) > 0
...
#             if 'invalid' in input_data.lower():
#                 return any(w in output_lower for w in ['invalid', 'not registered'])
#             candidate = il[1] if len(il) > 1 else ''
...
#     except Exception:
#         # Verifier crashed due to unexpected input — fall back to string check
#         exp = (expected_output or '').strip()
#         return exp.lower() in output_lower if exp else len(output) > 0

#     # Unknown problem title — fall back to string check
#     exp = (expected_output or '').strip()
#     return exp.lower() in output_lower if exp else len(output) > 0
